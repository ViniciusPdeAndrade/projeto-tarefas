package br.com.andrade.vpd.tarefas.sistema;
...
public class Paginador {
	...
	public Paginador(Page<?> lista, ParametroData entrada) {
		...
		if (semRegistro) {
			this.resumo = "NÃ£o foram encontrados registros para o filtro informado";
		} else {
			int primeiro = lista.getNumber() * lista.getSize() + 1;
			int ultimo = primeiro + lista.getNumberOfElements() - 1;
			this.resumo = String.format("Registros %d-%d de %d, em %d pÃ¡gina%s", primeiro, ultimo, total, totalPages, totalPages == 1 ? "" : "s");
		}
	}
